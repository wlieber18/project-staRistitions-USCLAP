...
We are concerned about spatial correlation, so we will split the data 80/20 to minimize this effect.
...
data_split <- initial_split(bll_data, prop = 0.80)
...
bll_data_modeltrain <- bll_train %>% 
  filter(num_bll != 0)

bll_full_model <- lm(imprisonment_rt ~ perc_bll_indicator + POC_other + med_income + median_age + perc_male + med_income * POC_other, data = bll_data_modeltrain) 
...
bll_data_modeltrain2 <- bll_train %>%
  mutate(
...
log_model <- lm(log_imprisonment_rt ~ log_med_income + perc_bll_indicator + 
                POC_other + median_age + perc_male, data = bll_data_modeltrain2)
