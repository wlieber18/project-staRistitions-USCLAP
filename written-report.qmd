...
data_split <- initial_split(bll_data_transformed, prop = 0.20)
...
bll_test_results <- bll_test %>%
  mutate(predicted = exp(log_imprisonment_rt)) %>%
  select(imprisonment_rate, predicted)

bll_test <- bll_test %>%
  mutate(imprisonment_rate = exp(log_imprisonment_rt))

metrics(bll_test_results, truth = imprisonment_rate, estimate = predicted)
```
