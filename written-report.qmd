...
We are concerned about spatial correlation, so we will split the data 80/20 to minimize this effect. 20% goes to training data.
...
data_split <- initial_split(bll_data_transformed, prop = 0.50)
bll_train <- training(data_split)
bll_test  <- testing(data_split)

```

```{r}
#| label: removing-influential-points
#removing influential points in the training data only.

#new model fitted on training data only

log_model_actual <- lm(log_imprisonment_rt ~ log_med_income + perc_bll_indicator + 
                POC_other + median_age + perc_male, data = bll_train)
...
#It does not appear there are any data points in the training data with a cook's distance greater than 0.5. Therefore, none are removed.
```
```{r}
...
stepwise_model_mass <- stepAIC(log_model_actual, direction = "both", trace = 1)

#This code will force R to keep BLL in the model #stepwise_model_forced <- stepAIC(
#  model_with_lead,
#  scope = list(
#    lower = ~ perc_bll_indicator,  # minimum model includes lead
#    upper = ~ perc_bll_indicator + log_med_income + POC_other + median_age + perc_male
#  ),
#  direction = "both",
#  trace = 1
#)
...
#Interestingly, the lead predictor was removed through stepwise selection, perhaps indicating that it is not useful here. We are going to force it back into the model since it is a variable of interest.
...
#R squared valued is abysmal. it is 0.029. This differs significantly from the R squares and adjusted R of models used on the training data.

# Full model R-squared
summary(log_model_actual)$r.squared
summary(log_model_actual)$adj.r.squared

# Stepwise model R-squared
summary(stepwise_model_mass)$r.squared
summary(stepwise_model_mass)$adj.r.squared

summary(log_model_checking)$r.squared
summary(log_model_checking)$adj.r.squared
```
