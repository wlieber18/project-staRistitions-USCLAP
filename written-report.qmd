...
The shape of the imprisonment rate distribution is unimodal that is skewed to the right with a median incarceration rate of `r carc_med` out of 100,000. This doesn't appear surprising - there are fewer census tracts with particularly high imprisonment rates.
...
Similar to our imprisonment rate distrubution, the shape of the BLL distribution is unimodal that is skewed to the right with a median rate of `r bll_med`. This doesn't appear surprising - there are fewer census tracts with particularly high BLL rates among children.
...
bll_data |>
  ...
  ggplot(aes(x = perc_bll_indicator, y = imprisonment_rt)) +
  geom_point(size = 0.4, alpha = 0.3, color = "blue4") +
  scale_x_continuous(limits = c(0, 30)) +
  scale_y_continuous(limits = c(0, 1500)) +
  labs(
    x = "Imprisonment Rate (per 100,000)",
    y = "Children under 6 with a BLL ≥3.5 µg/dL (%)",
    title = "Imprisonment Rate vs BLL Rate In CA",
    subtitle = "by Census Tract",
    caption = "Axis Limits Fixed for Clarity"
  )
```

From the above graph, the nature of the relationship between blood level and imprisonment rates isn't clearly defined.

**Income and Imprisonment Rate**
...
bll_data |>
  filter(perc_bll_indicator != 0) |>
  ggplot(aes(x = med_income, y = imprisonment_rt)) +
  geom_point(size = 0.4, alpha = 0.3, color = "blue4") +
  scale_y_continuous(limits = c(0, 2000)) +
  labs(
    x = "Imprisonment Rate (per 100,000)",
    y = "Median Income ($)",
    title = "Imprisonment Rate vs Median Income In CA",
    subtitle = "by Census Tract",
    caption = "Capturing ~95% of data along x- and y-axis"
  )
```

From the above graph, it seems there is a potentially nonlinear relationship between median income and imprisonment rates in census tracts.

**Potential Interactions**

For future analysis, we plan to explore if race and age can have added effects on our predictor variables. However, that will require further careful transformations of our current dataset.
